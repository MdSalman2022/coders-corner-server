...
const getFollowingUsers = async (req, res) => {
  try {
    const { id } = req.params; // Better Auth ID
    const { limit = 50 } = req.query; // Optional limit for pagination

    // Find user by Better Auth ID
    const user = await User.findOne({ betterAuthId: id }).populate({
      path: "following",
      select: "_id name avatar email betterAuthId", // Only fetch needed fields
    ...
    // Return following users with simplified data
    res.json({
      following: user.following.map((followedUser) => ({
        ...
        betterAuthId: followedUser.betterAuthId,
      })),
